// I copied this from an npm package because importing it wasn't working
// and changed literally the bare minimum to make it functional
// TODO: add types and clean all linting problems

// @ts-nocheck
function fromImageArray(images, fps, outputAsArray) {
  return toWebM(images.map(image => {
    const webp = parseWebP(parseRIFF(atob(image.slice(23))));
    (webp as any).duration = 1000 / fps;
    return webp;
  }), outputAsArray);
}
...
function toFlatArray(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
      if (typeof arr[i] == 'object') {
        // an array
        outBuffer.push(toFlatArray(arr[i]));
      } else {
        // a simple element
...
// 		if (typeof duration == 'undefined' && !this.duration) { throw new Error("if you don't have the fps set, you ned to have durations here.") }
